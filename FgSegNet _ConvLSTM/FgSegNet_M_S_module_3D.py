...
from keras.layers import Activation, Input, Dropout, BatchNormalization, SpatialDropout3D
from keras.layers.convolutional import Conv3D, Conv3DTranspose, UpSampling3D, Cropping3D
...
class FgSegNet_M_S_module(object):
    ...
    def VGG16(self, x): 
        ...
        # Block 1
        x = Conv3D(64, 3, activation='relu', padding='same', name='block1_conv1', data_format='channels_last')(x)
        
        x = Conv3D(64, 3, activation='relu', padding='same', name='block1_conv2')(x)
        
        x = MaxPooling3D( (1,2,2), strides= (1,2,2), name='block1_pool')(x)
        
        
        # Block 2
        x = Conv3D(128, 3, activation='relu', padding='same', name='block2_conv1')(x)
        
        x = Conv3D(128, 3, activation='relu', padding='same', name='block2_conv2')(x)
        
        x = MaxPooling3D((1,2,2), strides= (1,2,2), name='block2_pool')(x)
                
        # Block 3
        x = Conv3D(256, 3, activation='relu', padding='same', name='block3_conv1')(x) 
        
        x = Conv3D(256, 3, activation='relu', padding='same', name='block3_conv2')(x)
        
        x = Conv3D(256, 3, activation='relu', padding='same', name='block3_conv3')(x)
        
    
        # Block 4
        x = Conv3D(512, 3, activation='relu', padding='same', name='block4_conv1')(x)
        x = Dropout(0.5, name='dr1')(x)
        x = Conv3D(512, 3, activation='relu', padding='same', name='block4_conv2')(x)
        x = Dropout(0.5, name='dr2')(x)
        x = Conv3D(512, 3, activation='relu', padding='same', name='block4_conv3')(x)
    ...
    def transposedConv(self, x):
        ...
        # block 5
        x = Conv3DTranspose(64, 1, activation='relu', padding='same', name='block5_tconv1', 
                                                kernel_regularizer=regularizers.l2(self.reg))(x)     
        x = Conv3DTranspose(64, 3, activation='relu', padding='same', name='block5_tconv2')(x)       
        x = Conv3DTranspose(512, 1, activation='relu', padding='same', name='block5_tconv3')(x)
        
        # block 6
        x = Conv3DTranspose(64, 1, activation='relu', padding='same', name='block6_tconv1', 
                                                kernel_regularizer=regularizers.l2(self.reg))(x)
        x = Conv3DTranspose(64, 5, strides=(1, 2, 2), activation='relu', padding='same', name='block6_tconv2')(x)
        x = Conv3DTranspose(256, 1, activation='relu', padding='same', name='block6_tconv3')(x)
        
        # block 7
        x = Conv3DTranspose(64, 1, activation='relu', padding='same', name='block7_tconv1', 
                                                kernel_regularizer=regularizers.l2(self.reg))(x)
        x = Conv3DTranspose(64, 3, activation='relu', padding='same', name='block7_tconv2')(x)
        x = Conv3DTranspose(128, 1, activation='relu', padding='same', name='block7_tconv3')(x)
               
        # block 8
        x = Conv3DTranspose(64, 5, strides=(1,2, 2), activation='relu', padding='same', name='block8_conv1', 
                                                kernel_regularizer=regularizers.l2(self.reg))(x)
        
        # block 9
        x = Conv3DTranspose(1, 1, padding='same', name='block9_conv1')(x)
    ...
    def initModel_M(self, dataset_name):
        ...
        f, h, w, d = self.img_shape
        
        input_1 = Input(shape=(f, h, w, d), name='ip_scale1')
...
#                layer.trainable = False
        ...
        # Scale 2
        input_2 = Input(shape=(f, int(h/2), int(w/2), d), name='ip_scale2')
        x2 = shared_model(input_2)
        x2 = UpSampling3D((1,2,2))(x2)
        # Scale 3
        input_3 = Input(shape=(f, int(h/4), int(w/4), d), name='ip_scale3')
        x3 = shared_model(input_3)
        x3 = UpSampling3D((1,4,4))(x3)
        ...
        if dataset_name=='CDnet':
            ...
            if(self.scene=='wetSnow'):
                x1 = Cropping3D(cropping=((1, 2),(0, 0)))(x1)
            elif(self.scene=='skating'):
                x1 = Cropping3D(cropping=((1, 1),(1, 2)))(x1)
            else:
            ...
            for key, val in x2_ups.items():
                if self.scene == key and self.scene in ['skating', 'wetSnow']:
                    x2 = Cropping3D(cropping=((1, 1), val))(x2)
                    break
                elif self.scene==key:
...
#        top = self.features(top)
        ...
        if dataset_name=='CDnet':
            if(self.scene=='tramCrossroad_1fps'):
                ...
                top = Cropping3D(cropping=((0, 0),(0, 1)))(top)
            elif(self.scene=='tramStation'):  
                top = Cropping3D(cropping=((1, 0),(0, 0)))(top)
    ...
    def initModel_S(self, dataset_name):
        ...
        input_1 = Input(shape=(f, h, w, d), name='input')
        vgg_layer_output = self.VGG16(input_1)
...
#                layer.trainable = False
        ...
        if dataset_name=='CDnet':
            if(self.scene=='tramCrossroad_1fps'):
                ...
                x = Cropping3D(cropping=((0, 0),(0, 1)))(x)
            elif(self.scene=='tramStation'):  
                x = Cropping3D(cropping=((1, 0),(0, 0)))(x)
            elif(self.scene=='twoPositionPTZCam'):
