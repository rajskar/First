...
#    X_list = sorted(glob.glob(os.path.join(dataset_dir, 'input','*.jpg')))
    ...
    feature_dir = '/home/rajs/ML_DL/Background Subtractor/FgSegNet-master/RNN_Model/features'
    
    Y_list = sorted(glob.glob(os.path.join(dataset_dir, 'groundtruth', '*.png')))
    X_list = sorted(glob.glob(os.path.join(feature_dir,'*.npy')))
...
#        i = i + 500
        x = np.load(X_list[i])
        s = x.shape
        X.append(x.reshape(s[1], s[2], s[3]))
...
def train(results, scene, mdl_path, vgg_weights_path, method_name):
    ...
    if method_name=='FgSegNet_M':
        model.fit(results[0], results[1], validation_split=val_split, epochs=max_epochs, batch_size=batch_size, 
                           callbacks=[redu, chk], verbose=1, class_weight=results[2], shuffle = True)
    else:
...
#    tf.train.write_graph(frozen_graph, './', 'fgsegnet.pbtxt', as_text=True)
#    tf.train.write_graph(frozen_graph, './', 'fgsegnet.pb', as_text=False)
